      ...
      <div className="space-y-4">
        {faqs.map((faq, index) => (
          <div key={index} className="border border-blue-200 rounded-lg overflow-hidden bg-white shadow-md">
            ...
            >
              <span className="font-medium text-lg text-white-800">{faq.question}</span>
              <svg className={`w-6 h-6 text-white-600 transform transition-transform ${activeIndex === index ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {activeIndex === index && (
              <div className="p-6 bg-blue-50 border-t border-white-200">
                <p className="text-white-900">{faq.answer}</p>
